...
interactive and cannot be proxied over HTTP. The app installs `openssh-client`
and `rsync` into the *workspace* container on activation. An agent-runner
container is a different image: `ssh` is normally there, `rsync` may not be. If
you hit that, run the transfer from a workspace terminal, or use
`aw-workspace-cli remote-hosts push/pull` (aw-app-remote-host-cli) for the
BYOD host specifically.
